...
void CsvConverter::convertFile(string outputName, string outputIndex){
    string line;
    ifstream tsv(filename);
    ofstream output(outputName);
    ofstream indexOutput(outputIndex, ofstream::binary);
    int i = 0;
    int length = 0;
    indexOutput.write((char *)&length, sizeof(int));
